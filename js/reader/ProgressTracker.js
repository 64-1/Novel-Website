(function (global) {
  if (global.NovelReaderProgressTracker) {
    return;
  }
  ...
  function createTracker({ container, progressBar, progressFill, context = "reader" } = {}) {
    ...
    function scheduleUpdate(force = false) {
      ...
      if (remaining <= 0) {
        ...
        trailingTimeout = global.setTimeout(() => {
          trailingTimeout = null;
          runUpdate();
        }, remaining);
      }
  ...
  global.NovelReaderProgressTracker = Object.freeze({
    createTracker
  });
})(window);
